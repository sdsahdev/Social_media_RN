...
export default function VideoCalling({}) {
  ...
  const socket = SocketIOClient('http://192.168.29.140:3500', {
    //   const socket = SocketIOClient('https://webrc-video-backend.onrender.com/', {
  ...
  useEffect(() => {
    ...
    console.log(peerConnection.current, '====perconnecttion');
    return () => {
      socket.off('newCall');
    ...
    return (
      <View
        ...
        {remoteStream ? (
          ...
          console.log(remoteStream, '====null remote scrtem')
        )}
        <View
          style={{
            marginVertical: 12,
